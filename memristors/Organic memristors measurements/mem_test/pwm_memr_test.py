...
if __name__ == '__main__':
    ...
    l = logging.getLogger('pwm_memr_test_stand')
    ...
    gen.set_square_signal(source=2, dutycycle=30, vhigh=3, vhigh_unit="V", vlow=0, vlow_unit="v", period=10, period_unit="ms")

    times=[i*0.001 for i in range(100)]

    for t in times:
        l.debug("dt={:.4f}".format(t))
        gen.start_source(1)
        time.sleep(t)
        gen.start_source(2)
        time.sleep(2)
